...
/* --- RH helpers (novos) --- */
export function signRhToken(payload: { sub: number; email?: string; empresaId?: number }) {
  return jwt.sign(
    {
      sub: payload.sub,
      email: payload.email,
...
export function verifyRhToken(token: string) {
  try {
    const decoded = jwt.verify(token, SECRET as string) as any;
    // optionally validate role
    if (decoded.role && decoded.role !== 'rh') {
      return { ok: false, error: new Error('role mismatch') };
    }
    return { ok: true, payload: decoded as { sub: number; email?: string; empresaId?: number; role?: string; iat?: number; exp?: number } };
  } catch (err: any) {
    return { ok: false, error: err };
  }
}
