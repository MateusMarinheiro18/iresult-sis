              ...
              {modulos.map(modulo => (
                <div key={modulo.id} onClick={() => onSelectModulo(modulo.id)} style={{ padding: 16, background: '#FAFAFA', borderRadius: 12, cursor: 'pointer', transition: 'all 0.2s', border: '1px solid transparent' }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = '#F5F5F7'; (e.currentTarget as HTMLElement).style.borderColor = '#e0e0e0'; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = '#FAFAFA'; (e.currentTarget as HTMLElement).style.borderColor = 'transparent'; }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, height: 8 }}>
                    <div style={{ flex: modulo.classificacao === 'FAVORAVEL' ? 1 : 0.3, background: modulo.classificacao === 'FAVORAVEL' ? '#4CAF50' : '#E0E0E0', borderRadius: 4, transition: 'all 0.3s' }} />
                    <div style={{ flex: modulo.classificacao === 'INTERMEDIARIO' ? 1 : 0.3, background: modulo.classificacao === 'INTERMEDIARIO' ? '#FFC107' : '#E0E0E0', borderRadius: 4, transition: 'all 0.3s' }} />
                    <div style={{ flex: modulo.classificacao === 'RISCO' ? 1 : 0.3, background: modulo.classificacao === 'RISCO' ? '#F44336' : '#E0E0E0', borderRadius: 4, transition: 'all 0.3s' }} />
                  </div>
                </div>
              ))}
            </div>
