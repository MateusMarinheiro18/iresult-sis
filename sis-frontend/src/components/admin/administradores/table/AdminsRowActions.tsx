...
export default function AdminRowActions({
  ...
  async function handleDelete() {
    ...
    const ok = await confirm({
      ...
      description: 'Tem certeza que deseja excluir este administrador?',
    ...
    const loadingId = toast.loading('Excluindo administrador…');
    ...
    try {
      ...
      if (!res.ok) {
        const msg = extractMessageFromBody(body) ?? 'Erro ao deletar administrador.';
        toast.error(msg, { id: loadingId });
        setLoading(false);
        return;
      }

      const successMsg = extractMessageFromBody(body) ?? 'Administrador deletado com sucesso.';
      toast.success(successMsg, { id: loadingId });

      setTimeout(() => router.refresh(), 800);
    } catch (err: any) {
      console.error('Erro ao deletar administrador', err);
      const msg = err?.message ?? 'Erro inesperado ao deletar administrador.';
      toast.error(msg, { id: loadingId });
      setLoading(false);
    } finally {
      setLoading(false);
      ...
      {open && (
        <div className="menu" role="menu">
          <button
            ...
            onClick={() => {
              setOpen(false);
              // rota de edição (você vai implementar)
              window.location.href = `/admin/administradores/${adminId}/edit`;
            }}
          >
            Editar
          </button>
