...
export default function AdminsEditForm({ initial }: { initial?: any }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!initial?.id) {
      toast.error('ID do administrador ausente — não é possível salvar.');
      return;
    }
    ...
    try {
      const res = await fetch(`/api/admins/${initial.id}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        console.error('PUT /api/admins/:id', { status: res.status, body });
        toast.error(body?.error ?? 'Erro ao atualizar administrador.', { id: loadingId });
        setSaving(false);
        return;
      }
      ...
      toast.error('Erro de rede ao atualizar administrador.');
      setSaving(false);
    }
  }
