...
export default function FileUploader({ onParsed, setMessage, setParsing }: Props) {
  ...
  async function handleFiles(files?: FileList | null) {
    ...
    try {
      ...
      if (name.endsWith('.csv')) {
        // try papaparse, fallback to FileReader parse simple
        try {
          ...
          // fallback: read as text and naive split (only if papaparse missing)
        ...
        try {
          ...
          console.error('Erro ao processar XLSX (verifique se "xlsx" está instalado):', errXlsx);
          setMessage?.('Erro ao processar arquivo Excel. Verifique se a dependência "xlsx" está instalada.');
        }
      } else {
      ...
      setMessage?.(err?.message ? String(err.message) : 'Erro ao processar o arquivo. Veja o console.');
    } finally {
      setParsing?.(false);
    }
  }
  ...
  function mapParsed(parsed: Record<string, any>[]) {
    return parsed.map((r, idx) => {
      ...
      for (const kRaw of Object.keys(r)) {
        ...
        // Mapeamento mais específico para evitar confusão
  ...
  function convertBrazilianDateToISO(dateStr: string): string {
    ...
    // Tenta match DD/MM/YYYY ou DD-MM-YYYY
    ...
    // Tenta parse do Excel (número serial)
