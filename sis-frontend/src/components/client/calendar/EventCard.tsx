...
function formatDateTime(iso: string) {
  // tenta interpretar ISO/local date - se for apenas YYYY-MM-DD, mostra só data
  const d = new Date(iso);
  const datePart = d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' });

  let timePart = '';
  if (!(d.getHours() === 0 && d.getMinutes() === 0)) {
    timePart = d.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
  }
...
export default function EventCard({ event }: Props) {
  const empresasLabel = event.empresas && event.empresas.length > 0 ? event.empresas.map((e) => e.razaoSocial).join(', ') : null;
  ...
  return (
    <article className="calendar-event-card">
      ...
      <div className="calendar-event-body">
        ...
        {event.detalhes && <p className="calendar-event-description">{event.detalhes}</p>}
      </div>
    </article>
  );
}
