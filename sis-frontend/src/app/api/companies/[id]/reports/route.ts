  ...
  try {
    ...
    if (!tituloRaw) {
      return NextResponse.json(
        { message: 'Título do relatório é obrigatório.' },
        { status: 400 }
      );
    }
    if (tituloRaw.length > 300) {
      return NextResponse.json(
        { message: 'Título muito longo (máx 300).' },
        { status: 400 }
      );
    }
    ...
    if (maybeReportId) {
      ...
      if (!existing || existing.idEmpresa !== companyId) {
        return NextResponse.json(
          { message: 'Relatório para atualizar não encontrado.' },
          { status: 404 }
        );
      }
    ...
    const msg =
      extractBodyText(err) ?? 'Erro interno ao criar/atualizar relatório.';
